  ...
  <div class="flex flex-col gap-3">
    ...
    <Card v-if="idea.repository || idea.repositoryActivity.isMissing || idea.repositoryActivity.events.length > 0">
      ...
      <CardContent class="flex flex-col gap-4 text-sm">
        <div
          ...
          <div class="min-w-0 flex-1">
            <div class="text-xs font-medium uppercase text-muted-foreground">
              {{ repositoryState.label }}
            </div>
            <div class="mt-1 break-words font-medium text-foreground">
              {{ repositoryState.title }}
            </div>
            <div class="mt-1 break-words text-muted-foreground">
              {{ repositoryState.description }}
            </div>
          </div>
        </div>
        ...
        <dl
          v-if="idea.repositoryActivity.lastPushedAtForHumans || idea.repositoryActivity.lastSyncedAtForHumans"
          class="grid grid-cols-1 gap-2 text-muted-foreground sm:grid-cols-2">
          <div
            v-if="idea.repositoryActivity.lastPushedAtForHumans"
            class="rounded-md border border-border px-3 py-2">
            <dt class="text-xs uppercase">
              Last pushed
            </dt>
            <dd class="mt-1 text-foreground">
              {{ idea.repositoryActivity.lastPushedAtForHumans }}
            </dd>
          </div>
          <div
            v-if="idea.repositoryActivity.lastSyncedAtForHumans"
            class="rounded-md border border-border px-3 py-2">
            <dt class="text-xs uppercase">
              Last synced
            </dt>
            <dd class="mt-1 text-foreground">
              {{ idea.repositoryActivity.lastSyncedAtForHumans }}
            </dd>
          </div>
        </dl>
        ...
        <div class="border-t border-border pt-4">
          <div class="flex items-center justify-between gap-3">
            ...
            <span
              v-if="hasRepositoryActivity"
              class="text-xs text-muted-foreground">
              {{ idea.repositoryActivity.events.length.toLocaleString() }} events
            </span>
          </div>
          ...
          <ol
            v-else
            :id="repositoryTimelineId"
            class="mt-3 flex flex-col gap-3"
            aria-label="Repository activity timeline">
            <li
              v-for="event in recentRepositoryEvents"
              :key="event.id"
          ...
          <div
            v-if="olderRepositoryEvents.length > 0"
            class="mt-3 flex flex-col gap-3">
            <Button
              type="button"
              variant="ghost"
            ...
            <ol
              ...
              <li
                v-for="event in olderRepositoryEvents"
                :key="event.id"
                class="grid grid-cols-[auto_1fr] gap-3">
                <span :class="['mt-0.5 inline-flex h-7 min-w-14 items-center justify-center rounded-full border px-2 text-[0.7rem] font-medium', repositoryEventMarkerClass(event)]">
                  {{ repositoryEventLabel(event) }}
