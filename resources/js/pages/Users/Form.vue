  ...
  <Card class="w-full max-w-4xl">
    ...
    <CardContent>
      <form :action="user?.routes.update" method="POST" class="flex flex-col gap-5" @submit.prevent="submitProfile">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <FormField id="password" label="New password" help="Leave blank to keep your current password. New passwords need at least 12 characters with letters and numbers." :validator="optionalPasswordValidator">
            <template #default="{ invalid, describedBy, feedbackClass }">
              <input id="password" name="password" type="password" :class="['h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40', feedbackClass]" autocomplete="new-password" minlength="12" maxlength="128" :aria-invalid="invalid || undefined" :aria-describedby="describedBy">
            </template>
          </FormField>
          <FormField id="password_confirmation" label="Confirm password" :validator="optionalPasswordConfirmationValidator">
            <template #default="{ invalid, describedBy, feedbackClass }">
              <input id="password_confirmation" name="password_confirmation" type="password" :class="['h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40', feedbackClass]" autocomplete="new-password" minlength="12" maxlength="128" :aria-invalid="invalid || undefined" :aria-describedby="describedBy">
            </template>
          </FormField>
        </div>
