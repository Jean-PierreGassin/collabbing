...
<template>
  <AuthMorphCard title="Login" :action="session.routes.login" :success-href="session.routes.ideas">
    <template #default="{ errorsFor, isSubmitting }">
      <CsrfField />
      <FormField id="username" label="Username" :validator="loginUsernameValidator" :external-errors="errorsFor('username')">
      ...
      <FormField id="password" label="Password" :validator="loginPasswordValidator" :external-errors="errorsFor('password')">
        <template #default="{ invalid, describedBy, feedbackClass }">
