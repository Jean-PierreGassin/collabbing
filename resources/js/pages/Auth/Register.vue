      ...
      <form method="POST" :action="session.routes.register" class="flex flex-col gap-4">
        ...
        <FormField id="username" label="Username" help="Use 3-20 letters, numbers, dashes, or underscores." :validator="usernameValidator">
          <template #default="{ invalid, describedBy, feedbackClass }">
            <input id="username" type="text" :class="['h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40 sm:max-w-sm', feedbackClass]" name="username" autocomplete="username" minlength="3" maxlength="20" pattern="[A-Za-z0-9_-]+" :value="oldInputString('username')" :aria-invalid="invalid || undefined" :aria-describedby="describedBy" required autofocus>
          </template>
        </FormField>
        <div class="grid gap-4 sm:grid-cols-2">
          <FormField id="first_name" label="First name" :validator="personNameValidator">
            <template #default="{ invalid, describedBy, feedbackClass }">
              <input id="first_name" type="text" :class="['h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40', feedbackClass]" name="first_name" autocomplete="given-name" maxlength="255" :value="oldInputString('first_name')" :aria-invalid="invalid || undefined" :aria-describedby="describedBy" required>
            </template>
          </FormField>
          <FormField id="last_name" label="Last name" :validator="personNameValidator">
            <template #default="{ invalid, describedBy, feedbackClass }">
              <input id="last_name" type="text" :class="['h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40', feedbackClass]" name="last_name" autocomplete="family-name" maxlength="255" :value="oldInputString('last_name')" :aria-invalid="invalid || undefined" :aria-describedby="describedBy" required>
            </template>
          </FormField>
        </div>
        <FormField id="email" label="Email address" :validator="emailValidator">
          <template #default="{ invalid, describedBy, feedbackClass }">
            <input id="email" type="email" :class="['h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40', feedbackClass]" name="email" autocomplete="email" maxlength="255" :value="oldInputString('email')" :aria-invalid="invalid || undefined" :aria-describedby="describedBy" required>
          </template>
        </FormField>
        <div class="grid gap-4 sm:grid-cols-2">
